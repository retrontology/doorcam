...
impl SystemHealthManager {
    ...
    pub async fn update_component_health(&self, component: &str, health: ComponentHealth) {
        ...
        if health == ComponentHealth::Failed {
            ...
            if degradation.should_shutdown_on_failure(component) {
                error!("Essential component {} failed, system shutdown required", component);
                ...
                warn!("Optional component {} failed, continuing with degraded functionality", component);
            }
        }
    }
    ...
    pub async fn should_shutdown(&self) -> bool {
        ...
        for (component, health) in monitor.get_all_component_status() {
            if *health == ComponentHealth::Failed && degradation.should_shutdown_on_failure(component) {
                return true;
            }
        }
    ...
    async fn check_event_bus_health(event_bus: &Arc<EventBus>) -> ComponentHealth {
        ...
        match event_bus.publish(DoorcamEvent::SystemError {
...
impl SystemMetrics {
    ...
    pub fn record_error(&mut self, component: &str) {
        *self.component_error_counts.entry(component.to_string()).or_insert(0) += 1;
    }
    
    /// Record a successful recovery for a component
    pub fn record_recovery(&mut self, component: &str) {
        *self.component_recovery_counts.entry(component.to_string()).or_insert(0) += 1;
    }
    
    /// Get error count for a component
    pub fn get_error_count(&self, component: &str) -> u64 {
        self.component_error_counts.get(component).copied().unwrap_or(0)
    }
    
    /// Get recovery count for a component
    pub fn get_recovery_count(&self, component: &str) -> u64 {
        self.component_recovery_counts.get(component).copied().unwrap_or(0)
    }
    ...
    pub fn generate_report(&self) -> String {
        ...
        report.push_str(&format!("Time since reset: {:?}\n", self.time_since_reset()));
...
mod tests {
    ...
    async fn test_system_health_manager() {
        ...
        health_manager.update_component_health("camera", ComponentHealth::Healthy).await;
        health_manager.update_component_health("display", ComponentHealth::Degraded).await;
