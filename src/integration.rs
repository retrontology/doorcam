...
impl CameraRingBufferIntegration {
    ...
    pub async fn new(config: DoorcamConfig) -> Result<Self> {
        ...
        let estimated_capacity =
            (config.camera.fps as u64 * config.capture.preroll_seconds as u64 * 2) as usize;
...
mod tests {
    ...
    async fn test_integration_start_stop() {
        ...
        let start_result =
            tokio::time::timeout(Duration::from_millis(500), integration.start()).await;
        ...
        match start_result {
            Ok(Ok(())) => {
                ...
                let _result = integration
                    .wait_for_frames(Duration::from_millis(300))
                    .await;
                ...
                let stop_result =
                    tokio::time::timeout(Duration::from_millis(500), integration.stop()).await;
    ...
    async fn test_health_check() {
        ...
        match start_result {
            Ok(Ok(())) => {
                ...
                assert!(matches!(
                    health.status,
                    HealthStatus::Healthy | HealthStatus::Warning | HealthStatus::Unhealthy
                ));
    ...
    async fn test_restart_capture() {
        ...
        let integration = match CameraRingBufferIntegration::new(config).await {
            ...
            | Err(crate::error::DoorcamError::Camera(crate::error::CameraError::Configuration {
                ...
                println!(
                    "Camera hardware not available for testing - skipping restart capture test"
                );
                return;
            }
            Err(e) => panic!("Unexpected integration error: {}", e),
        ...
        let restart_result =
            tokio::time::timeout(Duration::from_secs(1), integration.restart_capture()).await;
