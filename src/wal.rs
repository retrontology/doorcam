...
impl WalWriter {
    ...
    pub async fn new(event_id: String, wal_dir: &Path) -> Result<Self> {
        ...
        tokio::fs::create_dir_all(wal_dir).await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to create WAL directory: {}", e)))?;
        ...
        let file = OpenOptions::new()
            ...
            .open(&wal_path)
            .await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to create WAL file: {}", e)))?;
        ...
        info!("Created WAL file for event {}: {}", event_id, wal_path.display());
        Ok(writer)
    }
    ...
    async fn write_header(&mut self) -> Result<()> {
        ...
        self.file.write_all(&header).await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to write WAL header: {}", e)))?;
    ...
    pub async fn append_frame(&mut self, frame: &FrameData) -> Result<()> {
        ...
        self.buffer.extend_from_slice(&timestamp_nanos.to_le_bytes());
    ...
    async fn flush_buffer(&mut self) -> Result<()> {
        if !self.buffer.is_empty() {
            self.file.write_all(&self.buffer).await
                .map_err(|e| DoorcamError::component("wal", &format!("Failed to write to WAL: {}", e)))?;
            
            debug!("Flushed {} bytes to WAL for event {}", self.buffer.len(), self.event_id);
            self.buffer.clear();
        }
        Ok(())
    }
    ...
    pub async fn close(mut self) -> Result<PathBuf> {
        ...
        self.flush_buffer().await?;
        self.file.flush().await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to flush WAL on close: {}", e)))?;
        
        // Update frame count in header
        let mut file = self.file.into_inner();
        file.seek(std::io::SeekFrom::Start(24)).await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to seek in WAL: {}", e)))?;
        
        file.write_all(&self.frame_count.to_le_bytes()).await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to update frame count: {}", e)))?;
        
        file.sync_all().await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to sync WAL on close: {}", e)))?;
        
        info!("Closed WAL for event {} ({} frames)", self.event_id, self.frame_count);
...
impl WalReader {
    ...
    pub async fn read_all_frames(&self) -> Result<Vec<FrameData>> {
        let mut file = File::open(&self.path).await
            .map_err(|e| DoorcamError::component("wal", &format!("Failed to open WAL file: {}", e)))?;
        ...
        if version != WAL_VERSION {
            return Err(DoorcamError::component("wal", &format!("Unsupported WAL version: {}", version)));
        }
        
        // Read frame count
        let frame_count = u32::from_le_bytes([header[24], header[25], header[26], header[27]]);
        
        info!("Reading {} frames from WAL: {}", frame_count, self.path.display());
        ...
        loop {
            ...
            match file.read_exact(&mut timestamp_buf).await {
                ...
                Err(e) => return Err(DoorcamError::component("wal", &format!("Failed to read timestamp: {}", e))),
            }
            ...
            file.read_exact(&mut id_buf).await
                .map_err(|e| DoorcamError::component("wal", &format!("Failed to read frame ID: {}", e)))?;
            let frame_id = u64::from_le_bytes(id_buf);
            
            // Read data length (4 bytes)
            let mut len_buf = [0u8; 4];
            file.read_exact(&mut len_buf).await
                .map_err(|e| DoorcamError::component("wal", &format!("Failed to read data length: {}", e)))?;
            let data_len = u32::from_le_bytes(len_buf) as usize;
            
            // Read JPEG data
            let mut data = vec![0u8; data_len];
            file.read_exact(&mut data).await
                .map_err(|e| DoorcamError::component("wal", &format!("Failed to read frame data: {}", e)))?;
...
pub async fn find_wal_files(wal_dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    let mut entries = tokio::fs::read_dir(wal_dir).await
        .map_err(|e| DoorcamError::component("wal", &format!("Failed to read WAL directory: {}", e)))?;
    
    while let Some(entry) = entries.next_entry().await
        .map_err(|e| DoorcamError::component("wal", &format!("Failed to read directory entry: {}", e)))? {
        let path = entry.path();
...
pub async fn delete_wal(wal_path: &Path) -> Result<()> {
    tokio::fs::remove_file(wal_path).await
        .map_err(|e| DoorcamError::component("wal", &format!("Failed to delete WAL file: {}", e)))?;
